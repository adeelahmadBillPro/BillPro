...
import { getCustomers, getNextInvoiceNumber, createInvoice, getActiveProducts, getInvoice, logActivity, deductStock } from "@/lib/supabase/database";
...
function NewInvoiceContent() {
  ...
  const loadData = useCallback(async () => {
    if (!business) return;
    const [custRes, nextNum, prodRes] = await Promise.all([
    ...
    if (duplicateId) {
      const { data: srcInv } = await getInvoice(duplicateId);
      if (srcInv) {
        setCustomerId(srcInv.customer_id);
        setTaxPercentage(Number(srcInv.tax_percentage) || 0);
        setDiscount(Number(srcInv.discount) || 0);
        setNotesEn(srcInv.notes_en || "");
        setNotesUr(srcInv.notes_ur || "");
        const srcItems = (srcInv.items || []).map((item: any) => ({
  ...
  const handleSave = async () => {
    ...
    if (saveError) {
      ...
      // Deduct stock for items linked to products
      for (const item of items.filter(i => i.product_id && i.quantity > 0)) {
        await deductStock(item.product_id!, item.quantity);
      }
      if (user) await logActivity(business.id, user.id, "created", "invoice", invoiceNumber);
      showToast(lang === "ur" ? `انوائس ${invoiceNumber} بن گئی` : `Invoice ${invoiceNumber} created`, "success");
  ...
  return (
    <DashboardLayout>
      <div className="max-w-4xl space-y-6">
        ...
        {error && (
          <div className="bg-red-50 border border-red-200 text-danger text-sm p-3 rounded-lg flex items-center gap-2 animate-shake">
        ...
        <div className="bg-white rounded-2xl border border-gray-200/60 shadow-sm p-6 space-y-6">
                ...
                <div key={index} className="border border-gray-200 rounded-lg p-3 md:p-0 md:border-0 md:rounded-none">
                  ...
                  {products.length > 0 && (
                    <div className="mb-2 md:mb-1">
                      {index === 0 && <label className="block text-xs text-muted mb-1 md:hidden">{t("prod_pick", lang)}</label>}
                      <select
                        defaultValue=""
          ...
          <div className="flex flex-col-reverse sm:flex-row items-center justify-end gap-3 border-t border-gray-200 pt-4">
            <button onClick={() => router.push("/invoices")} className="w-full sm:w-auto px-4 py-2.5 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors">
